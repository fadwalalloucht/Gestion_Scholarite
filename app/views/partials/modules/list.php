<?php 
$can_addM = PageAccessManager::is_allowed('modules/add');
$can_ModM = PageAccessManager::is_allowed('modules/edit');
$can_ModS = PageAccessManager::is_allowed('modules/delete');
?>  
  
    <template id="modulesList">
        <section class="page-component">
            <div v-if="showheader" class="bg-light p-3 mb-3">
                <div class="container-fluid">
                    <div class="row ">
                        <div  class="col-sm-4 comp-grid" :class="setGridSize">
                            <h3 class="record-title">Modules</h3>
                        </div>
<?php
if ($can_addM){
?>
                        ...
                        <div  class="col-sm-3 comp-grid" :class="setGridSize">
                            <button  class="btn btn btn-primary btn-block " @click="$root.$emit('showPageModal' , {page:'modulesAdd',  sorttype: 'desc'})">
...
?>
                                            ...
                                            <tbody>
                                                <tr v-for="(data,index) in records">
                                                    <th v-if="multicheckbox" class="td-checkbox">
                                                        <label>
                                                            <input type="checkbox" :value="data.id_module" name="selectedid" v-model="selected" />
                                                        </label>
                                                    </th>
                                                    ...
                                                    <th class="td-btn">
                                                        <div >
                                                            <router-link v-if="viewbutton" class="btn btn-sm btn-outline-primary" title="View Record" :to="'/modules/view/' + data.id_module">
  ...
  if ($can_ModM) {
      ?>
                                                            ...
                                                            <router-link v-if="editbutton" class="btn btn-sm btn-outline-success" title="Edit This Record" :to="'/modules/edit/' + data.id_module">
...
if ($can_ModS){
?>     
                                                            <button  v-if="deletebutton" class="btn btn-outline-danger btn-sm" @click="deleteRecord(data.id_module,index)" title="Delete This Record">
                                                                <span v-show="deleting != data.id_module"><i class="fa fa-times"></i></span>
                                                                Effacer

   <?php
}
?>                                                             
                                                                <clip-loader :loading="deleting == data.id_module" color="#fff" size="14px"></clip-loader>
                                                            </button>
                                                        </div>
                                                    </th>
                                                </tr>
                                    ...
                                    <div v-if="showfooter" class="page-footer">
                                        ...
                                        <dataimport extensions="" buttontext="Importer des données" post-action="modules/import_data" v-if="importbutton"></dataimport>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
	var ModulesListComponent = Vue.component('modulesList', {
			...
			allSelected: function(){
				...
				if(this.allSelected == true){
					for (var i in this.records){
						var id = this.records[i].id_module;
