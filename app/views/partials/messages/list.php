                ...
                <div class="container-fluid">
                    <div class="row ">
                        <div  class="col-sm-4 comp-grid" :class="setGridSize">
                            <h3 class="record-title">Messages</h3>
                        </div>
                        <div  class="col-sm-3 comp-grid" :class="setGridSize">
                            <router-link v-if="addbutton" class="btn btn btn-primary btn-block" :to="'/messages/add'">
                            ...
                            <div  class=" animated fadeIn">
                                <div class="">
                                    <nav v-if="fieldname || filterMsgs.length" class="page-header-breadcrumbs mb-3" aria-label="breadcrumb">
                                        <ul class="breadcrumb m-0 p-2">
                                            <li v-if="fieldname" class="breadcrumb-item">
                                                <router-link class="text-capitalize" to="/messages">Messages</router-link>
                                            </li>
                                            ...
                                            <tbody>
                                                <tr v-for="(data,index) in records">
                                                    <th v-if="multicheckbox" class="td-checkbox">
                                                        <label>
                                                            <input type="checkbox" :value="data.id_message" name="selectedid" v-model="selected" />
                                                        </label>
                                                    </th>
                                                    <th v-if="listsequence" class="td-sno">{{index + 1}}</th>
                                                    <td><router-link :to="'/messages/view/' +  data.id_message">{{data.id_message}}</router-link></td>
                                                    <td> {{ data.libelle_message }} </td>
                                                    <td> {{ data.emmeteur }} </td>
                                                    <td> {{ data.recepteur }} </td>
                                                    <th class="td-btn">
                                                        <div >
                                                            <button v-if="viewbutton" class="btn btn-sm btn-outline-primary" title="View Record" @click="showPageModal({page:'messagesView',  id:data.id_message})">
                                                                <i class="fa fa-eye"></i> Vue
                                                            </button>
                                                            <button v-if="editbutton" class="btn btn-sm btn-outline-success" title="Edit This Record" @click="showPageModal({page:'messagesEdit', id: data.id_message})">
                                                                <i class="fa fa-edit"></i> Modifier
                                                            </button>
                                                            <button  v-if="deletebutton" class="btn btn-outline-danger btn-sm" @click="deleteRecord(data.id_message,index)" title="Delete This Record">
                                                                <span v-show="deleting != data.id_message"><i class="fa fa-times"></i></span>
                                                                Effacer
                                                                <clip-loader :loading="deleting == data.id_message" color="#fff" size="14px"></clip-loader>
                                                            </button>
                                                        </div>
                                                    </th>
                                                </tr>
                                    ...
                                    <div v-if="showfooter" class="page-footer">
                                        ...
                                        <dataimport extensions="" buttontext="Importer des données" post-action="messages/import_data" v-if="importbutton"></dataimport>
                                    </div>
                                </div>
                            </div>
                        </div>
			...
			allSelected: function(){
				...
				if(this.allSelected == true){
					for (var i in this.records){
						var id = this.records[i].id_message;
