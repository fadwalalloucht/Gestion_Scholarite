    <template id="examensList">
        <section class="page-component">
            <div v-if="showheader" class="bg-light p-3 mb-3">
                <div class="container-fluid">
                    <div class="row ">
                        <div  class="col-sm-4 comp-grid" :class="setGridSize">
                            <h3 class="record-title">Examens</h3>
                        </div>
                        <div  class="col-md-4 comp-grid" :class="setGridSize">
                            <button  class="btn btn-primary " @click="$root.$emit('showPageModal' , {page:'examensAdd',  sorttype: 'desc'})">
                            ...
                            <div  class=" animated fadeIn">
                                <div class="">
                                            ...
                                            <tbody>
                                                <tr v-for="(data,index) in records">
                                                    <th v-if="multicheckbox" class="td-checkbox">
                                                        <label>
                                                            <input type="checkbox" :value="data.id_examen" name="selectedid" v-model="selected" />
                                                        </label>
                                                    </th>
                                                    <th v-if="listsequence" class="td-sno">{{index + 1}}</th>
                                                    <td> {{ data.libelle_examen }} </td>
                                                    <td> {{ data.date }} </td>
                                                    <td> {{ data.heur_debut }} </td>
                                                    <td> {{ data.heur_fin }} </td>
                                                    <th class="td-btn">
                                                        <div >
                                                            <router-link v-if="viewbutton" class="btn btn-sm btn-outline-primary" title="View Record" :to="'/examens/view/' + data.id_examen">
                                                            <i class="fa fa-eye"></i> Vue
                                                            </router-link>
                                                            <router-link v-if="editbutton" class="btn btn-sm btn-outline-success" title="Edit This Record" :to="'/examens/edit/' + data.id_examen">
                                                            <i class="fa fa-edit"></i> Modifier
                                                            </router-link>
                                                            <button  v-if="deletebutton" class="btn btn-outline-danger btn-sm" @click="deleteRecord(data.id_examen,index)" title="Delete This Record">
                                                                <span v-show="deleting != data.id_examen"><i class="fa fa-times"></i></span>
                                                                Effacer
                                                                <clip-loader :loading="deleting == data.id_examen" color="#fff" size="14px"></clip-loader>
                                                            </button>
                                                        </div>
                                                    </th>
                                                </tr>
                                    ...
                                    <div v-if="showfooter" class="page-footer">
                                        ...
                                        <dataimport extensions="" buttontext="Importer des données" post-action="examens/import_data" v-if="importbutton"></dataimport>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
	var ExamensListComponent = Vue.component('examensList', {
		template: '#examensList',
			...
			allSelected: function(){
				...
				if(this.allSelected == true){
					for (var i in this.records){
						var id = this.records[i].id_examen;
