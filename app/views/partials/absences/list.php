    <template id="absencesList">
        <section class="page-component">
            <div v-if="showheader" class="bg-light p-3 mb-3">
                <div class="container-fluid">
                    <div class="row ">
                        <div  class="col-sm-4 comp-grid" :class="setGridSize">
                            <h3 class="record-title">Absences</h3>
                        </div>
                        <div  class="col-sm-3 comp-grid" :class="setGridSize">
                            <router-link v-if="addbutton" class="btn btn btn-primary btn-block" :to="'/absences/add'">
                            ...
                            <div  class=" animated fadeIn">
                                <div class="">
                                    <nav v-if="fieldname || filterMsgs.length" class="page-header-breadcrumbs mb-3" aria-label="breadcrumb">
                                        <ul class="breadcrumb m-0 p-2">
                                            <li v-if="fieldname" class="breadcrumb-item">
                                                <router-link class="text-capitalize" to="/absences">Absences</router-link>
                                            </li>
                                            ...
                                            <tbody>
                                                <tr v-for="(data,index) in records">
                                                    <th v-if="multicheckbox" class="td-checkbox">
                                                        <label>
                                                            <input type="checkbox" :value="data.id_absence" name="selectedid" v-model="selected" />
                                                        </label>
                                                    </th>
                                                    <th v-if="listsequence" class="td-sno">{{index + 1}}</th>
                                                    <td><router-link :to="'/absences/view/' +  data.id_absence">{{data.id_absence}}</router-link></td>
                                                    <td> {{ data.justifie }} </td>
                                                    ...
                                                    <th class="td-btn">
                                                        <div >
                                                            <router-link v-if="viewbutton" class="btn btn-sm btn-outline-primary" title="View Record" :to="'/absences/view/' + data.id_absence">
                                                            <i class="fa fa-eye"></i> Vue
                                                            </router-link>
                                                            <router-link v-if="editbutton" class="btn btn-sm btn-outline-success" title="Edit This Record" :to="'/absences/edit/' + data.id_absence">
                                                            <i class="fa fa-edit"></i> Modifier
                                                            </router-link>
                                                            <button  v-if="deletebutton" class="btn btn-outline-danger btn-sm" @click="deleteRecord(data.id_absence,index)" title="Delete This Record">
                                                                <span v-show="deleting != data.id_absence"><i class="fa fa-times"></i></span>
                                                                Effacer
                                                                <clip-loader :loading="deleting == data.id_absence" color="#fff" size="14px"></clip-loader>
                                                            </button>
                                                        </div>
                                                    </th>
                                                </tr>
                                    ...
                                    <div v-if="showfooter" class="page-footer">
                                        ...
                                        <dataimport extensions="" buttontext="Importer des données" post-action="absences/import_data" v-if="importbutton"></dataimport>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
	var AbsencesListComponent = Vue.component('absencesList', {
		template: '#absencesList',
		...
		props: {
			...
			routename : {
				type : String,
				default : 'absenceslist',
			},
			apipath : {
				type : String,
				default : 'absences/list',
			},
			tablestyle: {
				type: String,
			...
			allSelected: function(){
				...
				if(this.allSelected == true){
					for (var i in this.records){
						var id = this.records[i].id_absence;
